    ...
    <section className="rounded-[2rem] border border-white/12 bg-white/8 p-6 shadow-lg shadow-black/25 backdrop-blur-2xl">
      ...
      <div className="mt-5 space-y-4">
        {matches.length === 0 ? (
          ...
          matches.map((match) => (
            <article key={match.id || match.fullName} className="flex items-center justify-between gap-4 rounded-2xl border border-white/12 bg-white/8 p-4">
              <div className="flex min-w-0 items-center gap-3">
                {match.photoUrl ? (
                  ...
                  <div className="grid h-14 w-14 shrink-0 place-items-center rounded-2xl bg-[linear-gradient(135deg,#0f172a,#1d4ed8)] text-sm font-semibold text-white">
                    {match.fullName
                      .split(' ')
                      .map((part) => part[0])
                      .join('')
                ...
                <div className="min-w-0">
                  <p className="font-semibold text-white">{match.fullName}</p>
                  <p className="truncate text-sm text-white/65">
                    {match.profession}
                    {match.company ? ` · ${match.company}` : ''}
                  </p>
                  <p className="mt-1 text-xs text-white/45">Matched {new Date(match.matchedAt).toLocaleString()}</p>
                </div>
              </div>
